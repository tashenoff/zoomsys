...
const items = pricingData.advertisingStands || []
const opKeys = ['pockets', 'border', 'canvasStretch', 'subframe']
const operations = opKeys.map((key) => pricingData.additionalOperations?.[key]).filter(Boolean)

async function ensureColumns() {
  await pool.query(`
    ALTER TABLE advertising_stands_pricing
...
async function upsert(item, sortOrder) {
  const existing = await pool.query(`SELECT id FROM advertising_stands_pricing WHERE name=$1 ORDER BY id LIMIT 1`, [item.name])
  const params = [item.name, item.type || 'fixed', item.unit || 'шт', item.price ?? null,
    item.prices ? JSON.stringify(item.prices) : null, item.description || null, item.notes || null, sortOrder]
  if (existing.rows[0]) {
    await pool.query(`UPDATE advertising_stands_pricing SET name=$1, type=$2, unit=$3, price=$4, prices=$5, description=$6, notes=$7, sort_order=$8, is_active=true, updated_at=NOW() WHERE id=$9`, [...params, existing.rows[0].id])
    return 'updated'
  }
  await pool.query(`INSERT INTO advertising_stands_pricing (name, type, unit, price, prices, description, notes, sort_order) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`, params)
  return 'inserted'
}
...
async function upsertOperation(item, sortOrder) {
  ...
  const params = [item.name, item.type || null, JSON.stringify(item.applicableTo || ['advertising-stands']),
...
async function main() {
  ...
  for (let i = 0; i < operations.length; i++) opStats[await upsertOperation(operations[i], 4000 + i)]++
  const after = await pool.query(`SELECT COUNT(*)::int AS n FROM advertising_stands_pricing WHERE is_active=true`)
  const ops = await pool.query(`SELECT COUNT(*)::int AS n FROM additional_operations WHERE is_active=true AND applicable_to::text LIKE '%advertising-stands%'`)
  console.log(JSON.stringify({ stands: stats, operations: opStats, active: after.rows[0], opsActive: ops.rows[0] }, null, 2))
}
