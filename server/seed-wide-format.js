...
const wideFormatItems = pricingData.wideFormat || []
const wideFormatOperationKeys = [
  'bannerGluing',
  'bannerJoining',
  'wideFormatCutting',
  'meshStitching',
...
const wideFormatOperations = wideFormatOperationKeys
  .map((key) => pricingData.additionalOperations?.[key])
  .filter(Boolean)
...
async function upsertWideFormatItem(item, sortOrder) {
  const existing = await pool.query(
    `SELECT id FROM wide_format_pricing
     WHERE name=$1 AND COALESCE(category, '')=COALESCE($2, '')
     ORDER BY id LIMIT 1`,
    [item.name, item.category || null]
  )
...
async function main() {
  if (!process.env.DATABASE_URL) {
    throw new Error('DATABASE_URL не задан. Проверь server/.env')
  }
  ...
  const before = await pool.query(`
    SELECT
      COUNT(*) FILTER (WHERE is_active=true) AS wide_active,
      COUNT(*) FILTER (WHERE is_active=true AND category IN ('phaeton','mimaki','roland')) AS wide_new_groups,
      (SELECT COUNT(*) FROM additional_operations WHERE is_active=true AND applicable_to::text LIKE '%wide-format%') AS wide_ops
    FROM wide_format_pricing
  `)

  await pool.query(
    `UPDATE wide_format_pricing
     SET is_active=false, updated_at=NOW()
     WHERE category IS NULL AND name IN ('Баннер','Пленка','Холст','Бумага')`
  )

  const wideStats = { inserted: 0, updated: 0 }
  for (let i = 0; i < wideFormatItems.length; i++) {
    const result = await upsertWideFormatItem(wideFormatItems[i], i)
    wideStats[result]++
  }

  const opStats = { inserted: 0, updated: 0 }
  for (let i = 0; i < wideFormatOperations.length; i++) {
    const result = await upsertOperation(wideFormatOperations[i], 1000 + i)
    opStats[result]++
  }
  ...
  const groupRows = await pool.query(`
    ...
    FROM wide_format_pricing
    WHERE is_active=true AND category IN ('phaeton','mimaki','roland')
    GROUP BY category
    ORDER BY category
  `)
...
main()
  .catch((err) => {
    console.error('❌ seed-wide-format failed:', err.message)
    process.exitCode = 1
  })
  .finally(async () => {
    await pool.end()
