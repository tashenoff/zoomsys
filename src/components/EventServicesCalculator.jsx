        ...
        <form onSubmit={handleCalculate} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" min="0" step="1" value={quantity} onChange={(e) => setQuantity(e.target.value)} required className="w-full px-3 py-2 md:px-4 md:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
            </div>
            {effectiveCategory === 'rent' && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Дней аренды</label>
                <input type="number" min="1" step="1" value={days} onChange={(e) => setDays(e.target.value)} className="w-full px-3 py-2 md:px-4 md:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
              </div>
            )}
          </div>
          {isRangePrice && (
            <div className="p-3 md:p-4 bg-indigo-50 border-2 border-indigo-200 rounded-lg">
              ...
              <input type="number" min="0" step="0.01" value={manualPrice} onChange={(e) => setManualPrice(e.target.value)} placeholder="Например 50000" className="w-full px-3 py-2 md:px-4 md:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" />
            </div>
          )}
        ...
        <div className="bg-white rounded-lg shadow-md p-4 md:p-6">
          ...
          {calculation.note ? (
            ...
            <div className="space-y-3">
              <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-1"><span className="text-gray-600">Позиция:</span><span className="font-semibold text-right break-words">{calculation.label}</span></div>
